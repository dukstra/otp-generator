...
namespace Dukstra\Otp;

use Carbon\Carbon;
use Illuminate\Support\Str;
use Dukstra\Otp\Models\Otp as OtpModel;
...
class OtpGenerator
{
    ...
    public function validateUsingId(int $id, string $token): object
    {
        $otp = OtpModel::query()->where('id', $id)->first();

        return $this->validate($otp, $token);
    }

    public function validateUsingIdentifier(string $identifier, string $token): object
    {
        $otp = OtpModel::query()->where('identifier', $identifier)->first();
     ...
     * @return object
     */
    private function validate($otp, $token): object
    {
        if (! $otp) {
            return (object) [
                'status' => false,
